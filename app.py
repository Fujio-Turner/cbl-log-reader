...
if len(sys.argv) < 2:
    print("Error: Please provide config.json as a command-line argument (e.g., python3 app.py config.json)")
    exit(1)
...
# Set DEBUG from config, defaulting to False if not present
...
if missing_keys:
    ...
    if DEBUG: print("Required keys are: 'cb-cluster-host', 'cb-bucket-user', 'cb-bucket-user-password', 'cb-bucket-name'")
    exit(1)
...
def get_date_range():
    ...
    # Create a unique cache key based on the query (no parameters in this case)
    ...
    try:
        ...
        try:
            ...
            for row in result:
                ...
                data = {
                    'start_date': min_dt[:10],
                    'end_date': max_dt[:10]
                }
                # Cache the raw data (dictionary)
...
def get_chart_data():
    ...
    if search_term:
        ...
        print(escaped_search_term," | ",search_term)
    ...
    # Create a unique cache key by combining query and parameters
    # Convert query_params to a JSON string to ensure consistent ordering
    ...
    try:
        ...
        try:
            ...
            # Cache the raw data (list of dictionaries)
            cbCache.upsert(cache_key, data)
            return jsonify(data)
        except CouchbaseException:
            if DEBUG: print(traceback.format_exc())
